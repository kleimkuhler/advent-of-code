...
def group(A, n):
    ...
    return list(map(list, [A[x:x+n] for x in range(0, len(A), n)]))
...
def generator(pos, vel, accel):
    while True:
        pass

def collision_reduce(pos, vel, accel):
    while True:
        pass
...
with open('./input.txt') as f:
    ...
    for i, line in enumerate(f):
        p, v, a = group(list(map(int, re.findall(r'-?\d+', line))), 3)
